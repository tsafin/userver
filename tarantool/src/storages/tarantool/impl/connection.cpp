...
void EncodeFixMap(std::vector<uint8_t>& out, uint8_t count) {
    out.push_back(static_cast<uint8_t>(mp::kFixMapMin | count));
}

void EncodeArray(std::vector<uint8_t>& out, uint32_t count) {
    if (count <= 15) {
...
// Encode a JSON value as MsgPack
void EncodeJson(std::vector<uint8_t>& out, const formats::json::Value& v);
...
void EncodeJson(std::vector<uint8_t>& out, const formats::json::Value& v) {
    if (v.IsNull()) {
        ...
        // encode as float64 (MsgPack float64 = 0xcb)
...
std::string MpDecoder::DecodeStr(uint32_t len) {
...
formats::json::Value MpDecoder::DecodeArray(uint32_t count) {
...
formats::json::Value MpDecoder::DecodeMap(uint32_t count) {
    formats::json::ValueBuilder obj(formats::json::Type::kObject);
    for (uint32_t i = 0; i < count; ++i) {
        auto key = DecodeValue();
        auto val = DecodeValue();
        // Keys in IPROTO headers/bodies are integers; stringify them for JSON
        std::string key_str;
        if (key.IsString()) {
...
formats::json::Value MpDecoder::DecodeValue() {
    uint8_t b = Read8();
    if (b <= 0x7f) {
        // positive fixint
...
uint32_t DecodePreheaderLength(const uint8_t* buf) {
    // IPROTO preheader: 0xce <uint32 big-endian>
    if (buf[0] != 0xce)
